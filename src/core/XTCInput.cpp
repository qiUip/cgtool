...
XTCInput::XTCInput(const string &filename)
{
    ...
    if (status != exdrOK)
        throw std::runtime_error("Could not open input XTC for reading");

    x_ = new rvec[natoms_];
    if (openFile(filename))
        throw std::runtime_error("Error reading initial frame from XTC");
}
...
int XTCInput::openFile(const std::string &filename)
{
    ...
    if (!cubic)
        printf("NOTE: Input box is not cubic\n");
