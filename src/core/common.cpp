...
void Common::setHelpStrings(const std::string &version,
                          ...
                          const std::vector<std::string> &opt_files)
{
    ...
    for (const string &f : req_files)
        inputFiles_[f].name = cmd_parser.getStringArg(f);
    for (const string &f : opt_files)
        inputFiles_[f].name = cmd_parser.getStringArg(f);

    for (auto &item : inputFiles_)
        item.second.exists = file_exists(item.second.name);
    ...
    if (cmd_parser.getIntArg("frames") != 0)
        numFramesMax_ = cmd_parser.getIntArg("frames");
}
...
void Common::getResidues()
{
    ...
    for (int i = 1; i < num_residues; i++)
    {
        ...
        {
            residues_[i].start =
                residues_[i - 1].total_atoms + residues_[i - 1].start;
...
void Common::doMainLoop()
{
    ...
    while (!end)
    {
            ...
            !(frame_->readNext() && (untilEnd_ || currFrame_ < numFramesMax_));
        if (currFrame_ % updateFreq_[updateLoc_] == 0)
            updateProgress();
        currFrame_++;
        mainLoop();
    }
    ...
    if (numFramesMax_ == -1)
    {
        // Bitrate (in MiBps) of XTC input - only meaningful if we read whole
        // file
        const double bitrate =
            file_size(inputFiles_["xtc"].name) / (time * 1024 * 1024);
...
void Common::updateProgress()
{
    ...
    if (numFramesMax_ < 0)
        t_remain = (wholeXTCFrames_ - currFrame_) / fps;

    printf("Read %'10d frames @ %'d FPS %6.1fs remaining\r", currFrame_,
           static_cast<int>(fps), t_remain);
    std::flush(cout);
